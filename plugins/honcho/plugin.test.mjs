...
// `memory__context`, so several of the assertions below are not about "is this
...
test("contributes five http tool runnables with namespaced native slugs", () => {
	...
	for (const r of manifest.runnables) {
		...
		assert.ok(
			r.config.slug.startsWith("honcho__"),
			`slug ${r.config.slug} not namespaced`
		);
	}
	assert.ok(bySlug.has("honcho__chat"));
	assert.ok(bySlug.has("honcho__search"));
	...
	assert.ok(bySlug.has("honcho__messages"));
	assert.ok(bySlug.has("honcho__session_upsert"));
	assert.ok(bySlug.has("honcho__peer_upsert"));
});
...
test("the dialectic tool does NOT expose `stream`", () => {
	...
	const props = bySlug.get("honcho__chat").config.input_schema.properties;
...
test("reasoning_level is pinned fast by body_defaults", () => {
	...
	// Dialectic pass routinely costs more, which would make `memory__context`
	// resolve and then silently never arrive.
	const chat = bySlug.get("honcho__chat").config;
...
test("search declares no `filters` argument", () => {
	...
	const props = bySlug.get("honcho__search").config.input_schema.properties;
...
test("memory__forget stays unbound — Honcho has no delete endpoint", () => {
	...
	assert.equal(byCapability.get("memory").tools.memory__forget, undefined);
});
...
test("the write verbs are ADAPTED, because the peer id must nest inside messages[]", () => {
	...
	for (const verb of ["memory__sync", "memory__store"]) {
		const binding = tools[verb];
		assert.equal(binding.tool, "honcho__messages");
...
test("an assistant turn is never attributed to the user's peer", () => {
	...
	const code = byCapability.get("memory").tools.memory__sync.adapter.code;
...
test("no verb declares an arg_clamp", () => {
	// Honcho's `limit` maxes at 100 and the canonical `memory__search` limit at 50,
...
test("memory__context declares NO response map, on purpose", () => {
	...
	const binding = byCapability.get("memory").tools.memory__context;
	assert.equal(binding.response, undefined);
	assert.equal(binding.tool, "honcho__chat");
});

test("memory__context pins workspace, peer and reasoning from preferences", () => {
	const binding = byCapability.get("memory").tools.memory__context;
...
test("memory__search normalizes into the canonical item shape", () => {
	const binding = byCapability.get("memory").tools.memory__search;
...
test("memory__search drops `scope`, which Honcho has no concept of", () => {
	const binding = byCapability.get("memory").tools.memory__search;
...
test("the reasoning-level select agrees with the documented enum and the body default", () => {
	...
	assert.equal(
		field.default,
		bySlug.get("honcho__chat").config.body_defaults.reasoning_level
	);
});
