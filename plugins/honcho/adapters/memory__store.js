// Capability adapter for `memory / memory__store`, run in Core's plugin sandbox.
...
// Same narrow repair gate as memory__sync: only a missing session or peer is
// fixable by upserting, and this path runs on every builtin memory write.
if (res && (res.status === 404 || res.status === 422)) {
	await callNamed("honcho__peer_upsert", { workspace_id: ws, id: user });
	await callNamed("honcho__session_upsert", {
		workspace_id: ws,
		id: session,
	});
