// Co-located contract test for the `rtk` plugin (plugins-store/plugins/rtk).
// Zero-dependency, runnable with:  node --test plugins-store/plugins/rtk/plugin.test.mjs
//
// rtk is a COMMAND-TOOL plugin: it exposes a single `command` backend runnable
// (`rtk.run`) that shells out to the locally-installed `rtk` binary. It has no
...
test("args mapping is coherent: mode->flags map covers every enum, command splits shell", () => {
	...
	for (const value of modeEnum) {
		assert.ok(Object.hasOwn(modeArg.map, value), `mode map covers "${value}"`);
		assert.ok(
			Array.isArray(modeArg.map[value]),
			`mode map[${value}] is an array`
		);
	}
...
test("contributes.settings_tabs is well-formed (RTK tab + pref-keyed fields)", () => {
	...
	for (const field of tab.fields) {
		assert.ok(
			...
			`field type "${field.type}" is known`
		);
		...
		assert.ok(
			!prefKeys.has(field.pref_key),
			`pref_key "${field.pref_key}" is unique`
		);
