...
//      Losing either turns the first `web.crawl {url}` call into a timeout.
...
test("contributes one http tool runnable with a namespaced native slug", () => {
	...
	assert.ok(bySlug.has("spidercloud.crawl"));
});
...
// `limit` is OPTIONAL, so without a default a bare `web.crawl {url}` asks for the
// whole site and blows the host's 30s http timeout, returning nothing at all.

const crawlConfig = bySlug.get("spidercloud.crawl").config;
...
test("the verb forwards to a tool this manifest actually declares", () => {
	assert.ok(
		...
		`web.crawl forwards to ${binding.tool}, which this manifest does not declare`
	);
});
...
test("argument renames target arguments the provider tool really accepts", () => {
	...
	for (const [canonical, target] of Object.entries(binding.args ?? {})) {
		...
		assert.ok(
			...
			`web.crawl maps ${canonical} onto '${field}', which ${binding.tool} does not accept`
		);
	}
});
