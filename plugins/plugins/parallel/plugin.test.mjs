...
test("exposes the native tool ids the bindings and adapter name", () => {
	...
	for (const slug of bySlug.keys()) {
		assert.ok(slug.startsWith("parallel."), `slug ${slug} not namespaced`);
	}
});

test("the KEYED tools carry a SERVER-SIDE x-api-key secret header from env", () => {
	// parallel.free_search deliberately has none: it is the no-key path, and
	// attaching a secret header to a public endpoint would leak the key to a host
	// that never needed it.
	assert.equal(
		bySlug.get("parallel.free_search").config.secret_headers,
		undefined
	);
	for (const r of manifest.runnables) {
		if (r.config.slug === "parallel.free_search") {
			continue;
		}
...
test("search routes to /v1/search, not the legacy beta path", () => {
	const s = bySlug.get("parallel.search");
...
test("extract routes to /v1/extract and asks for full page content", () => {
	const e = bySlug.get("parallel.extract");
...
test("input schemas are well-formed JSON Schema objects with required keys", () => {
	const s = bySlug.get("parallel.search").config.input_schema;
...
test("web.search is bound, and through an adapter", () => {
	...
	assert.deepEqual(binding.adapter.tools, ["parallel.free_search"]);
});

test("web.extract binds DECLARATIVELY — it needs no adapter", () => {
	const entry = manifest.provides.find((p) => p.capability === "web.extract");
	assert.ok(entry, "must provide web.extract");
	const binding = entry.tools["web.extract"];
	assert.equal(binding.tool, "parallel.extract");
...
test("the free endpoint is called statelessly, with no Accept header dance", () => {
	...
	const free = bySlug.get("parallel.free_search").config;
