...
test("exposes the native tool ids exa.search and exa.find_similar", () => {
	assert.ok(bySlug.has("exa.search"), "missing exa.search slug");
	assert.ok(bySlug.has("exa.find_similar"), "missing exa.find_similar slug");
	// slugs are namespaced under the plugin id
	for (const slug of bySlug.keys()) {
		assert.ok(slug.startsWith("exa."), `slug ${slug} not namespaced`);
	}
});

test("the KEYED tools carry a SERVER-SIDE Authorization secret header from env", () => {
	// exa.free_search deliberately has none: it is the no-key fallback, and
	// attaching a secret header to a public endpoint would leak the key to a host
	// that never needed it.
	assert.equal(bySlug.get("exa.free_search").config.secret_headers, undefined);
	for (const r of manifest.runnables) {
		if (r.config.slug === "exa.free_search") {
			continue;
		}
...
test("find-similar tool routes to /findSimilar with num_results default", () => {
	const f = bySlug.get("exa.find_similar");
...
test("input schemas are well-formed JSON Schema objects with required keys", () => {
	const s = bySlug.get("exa.search").config.input_schema;
	...
	const f = bySlug.get("exa.find_similar").config.input_schema;
...
test("permission_grants gate egress to the exa host only", () => {
	...
	assert.deepEqual(manifest.permission_grants, [
		...
		// web.search is bound through an adapter (it falls back to the keyless
...
test("web.search is bound, and through an adapter", () => {
	...
	assert.equal(binding.tool, "exa.search", "the KEYED tool is primary");
	...
	assert.deepEqual(binding.adapter.tools, ["exa.free_search"]);
});
...
test("the free endpoint is called with the Accept header it requires", () => {
	...
	const free = bySlug.get("exa.free_search").config;
