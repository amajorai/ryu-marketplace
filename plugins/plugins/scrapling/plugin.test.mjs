...
//      so dropping the grant breaks every `web.extract` call through this provider.
...
test("declares NO runnables — the tools are owned by the MCP server", () => {
	...
	// `app.<slug>` alias alongside its native `scrapling.<tool>` id.
...
test("verb key is the canonical dotted id under the right capability", () => {
...
test("the verb forwards to a tool the declared MCP server actually serves", () => {
	// An MCP tool registers as `<mcp_servers key>.<tool name>`. The binding is a
	...
	assert.equal(binding.tool, `${serverKey}.get`);
});
