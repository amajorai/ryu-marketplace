...
// Run with:  node --test plugins-store/plugins/pi-shell/plugin.test.mjs
//
...
test("publishes snapshots and consumes pinned-panel stop requests", () => {
	const source = readFileSync(
		join(HERE, manifest.contributes.pi_extensions[0].file),
		"utf8"
	);
	assert.match(source, /RYU_MCP_CORE_URL/);
	assert.match(source, /\/api\/background\/processes\/sync/);
	assert.match(source, /\/api\/background\/processes\/control/);
	assert.match(source, /background_process: background/);
	assert.match(source, /terminate\(\s*shell/);
});
