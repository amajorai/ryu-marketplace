...
test("has required top-level identity fields", () => {
	assert.equal(manifest.id, "@ryu/bytebot");
...
test("contributes six http tool runnables with namespaced native slugs", () => {
	...
	for (const r of manifest.runnables) {
		...
		assert.ok(
			r.config.slug.startsWith("bytebot."),
...
test("each tool pins its `action` constant in body_defaults", () => {
	...
	const expected = {
		...
		"bytebot.click": "click_mouse",
		"bytebot.type_text": "type_text",
		"bytebot.type_keys": "type_keys",
		"bytebot.scroll": "scroll",
		"bytebot.application": "application",
	};
...
test("only the screenshot unwraps its body; the action tools stay enveloped", () => {
	...
	assert.equal(bySlug.get("bytebot.screenshot").config.unwrap_body, true);
	for (const slug of [
		"bytebot.click",
		"bytebot.type_text",
		"bytebot.type_keys",
...
test("enum-valued arguments match the daemon's own enums exactly", () => {
	const click = bySlug.get("bytebot.click").config.input_schema.properties;
	assert.deepEqual(click.button.enum, ["left", "right", "middle"]);
	const scroll = bySlug.get("bytebot.scroll").config.input_schema.properties;
	assert.deepEqual(scroll.direction.enum, ["up", "down", "left", "right"]);
	// ApplicationName is a CLOSED enum — this is the whole reason computer.focus_app
	// is not bound, so the list living in the schema is load-bearing, not cosmetic.
	const app = bySlug.get("bytebot.application").config.input_schema.properties;
...
test("binds five canonical verbs and deliberately not focus_app", () => {
	...
	assert.equal(entry.tools["computer.focus_app"], undefined);
});
...
test("verbs forward to the tool whose action actually performs them", () => {
	...
	assert.equal(t["computer.capture"].tool, "bytebot.screenshot");
	assert.equal(t["computer.click"].tool, "bytebot.click");
	assert.equal(t["computer.type"].tool, "bytebot.type_text");
	assert.equal(t["computer.scroll"].tool, "bytebot.scroll");
	...
	assert.equal(t["computer.key"].tool, "bytebot.type_keys");
	assert.equal(
		bySlug.get(t["computer.key"].tool).config.body_defaults.action,
		"type_keys"
	);
});
...
test("click templates the coordinate pair the daemon nests", () => {
	const binding = byCapability.get("computer.control").tools["computer.click"];
...
test("scroll DROPS x/y instead of templating them", () => {
	const binding = byCapability.get("computer.control").tools["computer.scroll"];
...
test("scroll clamps amount, and click does not clamp count", () => {
	...
	assert.deepEqual(tools["computer.scroll"].arg_clamp, {
		amount: { min: 1, max: 10 },
	});
	...
	const scrollProps =
		bySlug.get("bytebot.scroll").config.input_schema.properties;
	...
	assert.equal(tools["computer.click"].arg_clamp, undefined);
});
...
test("no verb binding invents an argument the canonical schema lacks", () => {
	...
	const canonical = {
		"computer.capture": [],
		"computer.click": ["x", "y", "button", "count"],
		"computer.type": ["text"],
		"computer.key": ["keys"],
		"computer.scroll": ["x", "y", "direction", "amount"],
	};
