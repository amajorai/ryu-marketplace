...
// serving that verb.
//
// The WRITE half (`memory.store`, `memory.sync`) is bound through `arg_template`,
...
test("contributes three http tool runnables with namespaced native slugs", () => {
	...
	assert.ok(bySlug.has("mem0.search"));
	assert.ok(bySlug.has("mem0.add"));
	assert.ok(bySlug.has("mem0.delete"));
});
...
test("each tool uses the HTTP method Mem0 documents for its endpoint", () => {
	...
	assert.equal(bySlug.get("mem0.search").config.method, "POST");
	assert.equal(bySlug.get("mem0.add").config.method, "POST");
	assert.equal(bySlug.get("mem0.delete").config.method, "DELETE");
});

test("tool urls are the exact documented Mem0 paths", () => {
	assert.equal(
		bySlug.get("mem0.search").config.url,
	...
	assert.equal(
		bySlug.get("mem0.add").config.url,
		"https://api.mem0.ai/v3/memories/add/"
	);
	assert.equal(
		bySlug.get("mem0.delete").config.url,
...
test("delete keeps its status envelope while the two POSTs unwrap", () => {
	...
	assert.equal(bySlug.get("mem0.search").config.unwrap_body, true);
	assert.equal(bySlug.get("mem0.add").config.unwrap_body, true);
	assert.equal(bySlug.get("mem0.delete").config.unwrap_body, false);
});
...
test("mem0.add declares the documented add-memories request shape", () => {
	...
	//                               provided text as-is."
	const sch = bySlug.get("mem0.add").config.input_schema;
	...
	// response and feeding `event_id` (a job id) to mem0.delete (which wants a memory
	// id). No manifest field can prevent that, so the prose is load-bearing and a
	// rewrite must not silently drop it.
	assert.match(bySlug.get("mem0.add").config.description, /event_id/);
});
...
test("verb keys are canonical DOUBLE-underscore ids under the right capability", () => {
	...
	assert.deepEqual(
		...
		["memory.forget", "memory.search", "memory.store", "memory.sync"],
		"memory must bind search, store, sync and forget"
	);
});

test("memory.context stays deliberately unbound", () => {
	...
	assert.ok(
		!Object.hasOwn(memoryTools, "memory.context"),
		"memory.context is bound, but Mem0 has no standing-summary endpoint"
	);
});
...
test("search renames limit onto Mem0's top_k and drops the scope it has no concept of", () => {
	const binding = memoryTools["memory.search"];
...
test("forget renames the canonical id onto Mem0's path parameter", () => {
	const binding = memoryTools["memory.forget"];
...
test("memory.store templates the documented messages array", () => {
	...
	const binding = memoryTools["memory.store"];
	assert.equal(binding.tool, "mem0.add");
	assert.deepEqual(binding.arg_template, {
		messages: [{ role: "user", content: "{content}" }],
	});
	// The canonical arg the template consumes must be a real property of the verb —
	// `memory.store` is `{content, scope?, category?, importance?, when_to_use?}`.
	assert.match(binding.arg_template.messages[0].content, /^\{content\}$/);
});

test("memory.sync carries the turn's role through the template", () => {
	const binding = memoryTools["memory.sync"];
	assert.equal(binding.tool, "mem0.add");
	assert.deepEqual(binding.arg_template, {
		messages: [{ role: "{role}", content: "{content}" }],
	});
	// `role` is a PLACEHOLDER, not a pinned literal, and that is the decision worth
	// pinning. Canonical `memory.sync` is `{content, role?}`; an absent placeholder
...
test("sync declares no dead arg drops", () => {
	// Both canonical arguments of `memory.sync` are consumed by the template, and a
	// consumed key never reaches the rename pass — so any `args` entry here could
	// never fire.
	assert.equal(memoryTools["memory.sync"].args, undefined);
});
...
test("store skips Mem0's inference, sync delegates to it", () => {
	...
	//   memory.store = "a fact you have already decided on" -> store as-is.
	//   memory.sync  = "let IT decide what is worth remembering" -> Mem0's default.
	assert.equal(memoryTools["memory.store"].arg_defaults.infer, false);
	assert.equal(memoryTools["memory.sync"].arg_defaults.infer, undefined);
});
...
test("neither write verb maps the async job envelope onto a canonical field", () => {
	...
	// agent chain memory.forget onto a store result and delete nothing (or worse).
	...
	for (const verb of ["memory.store", "memory.sync"]) {
		assert.equal(
...
test("the entity id sits where Mem0 documents it on EACH endpoint", () => {
	...
	assert.deepEqual(memoryTools["memory.search"].arg_defaults, {
		filters: { user_id: "pref:mem0.user-id" },
	});
	assert.equal(
		memoryTools["memory.store"].arg_defaults.user_id,
		"pref:mem0.user-id"
	);
	assert.equal(
		memoryTools["memory.sync"].arg_defaults.user_id,
		"pref:mem0.user-id"
	);
	for (const verb of ["memory.store", "memory.sync"]) {
		assert.equal(
