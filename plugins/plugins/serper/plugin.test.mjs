...
// `web.search` / `web.extract` verbs to its own tools, so selecting it in the
...
test("has required top-level identity fields", () => {
	assert.equal(manifest.id, "@ryu/serper");
...
test("contributes two POST http tool runnables with namespaced native slugs", () => {
	...
	for (const r of manifest.runnables) {
		...
		assert.ok(
			r.config.slug.startsWith("serper."),
			`slug ${r.config.slug} not namespaced`
		);
	}
	assert.ok(bySlug.has("serper.search"));
	assert.ok(bySlug.has("serper.scrape"));
});
...
test("the tools target their two distinct serper.dev hosts over https", () => {
	const search = bySlug.get("serper.search").config;
	const scrape = bySlug.get("serper.scrape").config;
...
test("every declared argument is a real Serper request-body field", () => {
	...
	const bodyFields = {
		"serper.search": new Set(["q", "num", "page", "gl", "hl", "autocorrect"]),
		"serper.scrape": new Set([
			"url",
			"includeMarkdown",
			"includeImages",
			"includeLinks",
	...
	assert.deepEqual(bySlug.get("serper.search").config.input_schema.required, [
		"q",
	]);
	assert.deepEqual(bySlug.get("serper.scrape").config.input_schema.required, [
		"url",
	]);
});
...
test("the scrape tool asks for markdown by default, at the TOOL layer", () => {
	...
	// so a direct `serper.scrape` call gets markdown too — not only calls that
	// arrive through the `web.extract` facade.
	const cfg = bySlug.get("serper.scrape").config;
...
test("provides web.search and web.extract, but NOT web.crawl", () => {
	...
	// so it cannot serve `web.crawl`. Declaring the capability anyway would put it
...
test("BOTH canonical search arguments are renamed, none passed through", () => {
	...
	const { args } = byCapability.get("web.search").tools["web.search"];
