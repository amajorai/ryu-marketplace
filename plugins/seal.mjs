...
//
//   node plugins-store/goal/seal.mjs          # write
//   node plugins-store/goal/seal.mjs --check  # verify, non-zero on drift
//
	...
	if (process.argv.includes("--check")) {
		...
		if (bad.length > 0) {
			process.stderr.write(
				`manifest.json is out of date with tools/*.js (${bad.join(", ")}) — run: node plugins-store/goal/seal.mjs\n`
			);
			process.exit(1);
		}
