...
// `web__search` / `web__extract` verbs to its own tools, so selecting it in the layer
...
//
// The one binding this plugin deliberately does NOT make — `web__crawl` — is asserted
...
test("contributes four http tool runnables with namespaced native slugs", () => {
	...
	for (const r of manifest.runnables) {
		...
		assert.ok(
			r.config.slug.startsWith("firecrawl__"),
			`slug ${r.config.slug} not namespaced`
		);
	}
	assert.ok(bySlug.has("firecrawl__search"));
	assert.ok(bySlug.has("firecrawl__scrape"));
	// The crawl JOB pair: one call starts it, the other reads it.
	assert.ok(bySlug.has("firecrawl__crawl_start"));
	assert.ok(bySlug.has("firecrawl__crawl_status"));
});
...
test("the crawl job pair uses the verbs its two endpoints actually require", () => {
	...
	assert.equal(bySlug.get("firecrawl__crawl_start").config.method, "POST");
	const status = bySlug.get("firecrawl__crawl_status").config;
...
test("the two endpoints are the verified search and scrape routes", () => {
	assert.equal(
		bySlug.get("firecrawl__search").config.url,
		"https://api.firecrawl.dev/v2/search"
	);
	assert.equal(
		bySlug.get("firecrawl__scrape").config.url,
...
test("web.crawl is served through an ADAPTER, because the endpoint is async", () => {
	...
	// with no loop, so binding `web__crawl` that way would hand the model a UUID
	...
	assert.equal(binding.tool, "firecrawl__crawl_start");
	assert.ok(binding.adapter, "web__crawl must carry an adapter");
	...
	assert.deepEqual(binding.adapter.tools, ["firecrawl__crawl_status"]);
...
test("the crawl adapter returns pages, never a bare job id", () => {
	...
	const code = byCapability.get("web.crawl").tools.web__crawl.adapter.code;
	assert.match(code, /callNamed\(\s*"firecrawl__crawl_status"/);
...
test("canonical args that are neither renamed nor dropped pass through by name", () => {
	...
	// table: web__search{query,limit}, web__extract{url,format}.
	const canonical = {
		web__search: ["query", "limit"],
		web__extract: ["url", "format"],
		web__crawl: ["url", "depth", "limit"],
	};
	for (const p of provides) {
		for (const [verb, binding] of Object.entries(p.tools)) {
			if (binding.adapter) {
...
test("extract adapts the single scrape object, dotted through metadata", () => {
	...
	assert.equal(binding.tool, "firecrawl__scrape");
	assert.ok(binding.adapter, "web__extract must carry an adapter");
...
test("extract normalizes Firecrawl's string|string[] title IN ITS OWN adapter", () => {
	...
	const code = byCapability.get("web.extract").tools.web__extract.adapter.code;
	assert.match(
		code,
