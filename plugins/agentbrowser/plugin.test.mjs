...
// `npx -y agent-browser mcp`, not guessed.
...
test("mcp_servers.agentbrowser is a well-formed command spec", () => {
	...
	assert.deepEqual(server.args, ["-y", "agent-browser", "mcp"]);
...
// Captured from a live `tools/list` against `npx -y agent-browser mcp`. Core
// registers an MCP server's tools as `<server>__<tool>`, which is why every
// binding target below carries the `agentbrowser__` prefix.
