...
// binding the canonical `web__search` / `web__extract` verbs to its own tools, so
...
test("has required top-level identity fields", () => {
	assert.equal(manifest.id, "tavily");
...
test("contributes two http tool runnables with namespaced native slugs", () => {
	...
	for (const r of manifest.runnables) {
		...
		assert.ok(
			r.config.slug.startsWith("tavily__"),
			`slug ${r.config.slug} not namespaced`
		);
	}
	assert.ok(bySlug.has("tavily__search"));
	assert.ok(bySlug.has("tavily__extract"));
});
...
test("manifest is byte-identical to the Core fixture (registration seam)", () => {
	const fixturePath = resolve(
		here,
		"../../apps/core/src/plugin_manifest/fixtures/tavily.manifest.json"
	);
